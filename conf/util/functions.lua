...
--数组table洗牌 这个函数要搬到function
function table.shuffle(t,shuffle_times)
	if shuffle_times>#t then shuffle_times = #t end
	
	for i = 1,shuffle_times,1 do
		local member = t[i]
		local rand_index = randIntD(1,#t)
		--乾坤大挪移
		t[i] = t[rand_index]
		t[rand_index] = member
	end
