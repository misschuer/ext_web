...
function _M:match_result(result)
	...
	for _, matchInfo in pairs(result) do
		...
		--ngx.log(ngx.ERR, "match_result ", player_guid, " set match info ", matchInfo.info)
		cache:SetMatchResult(player_guid, matchInfo.info, true)
	end
end
...
--同步钱数据
function _M:sync_money(player_guid, isPk)
	local gold, changed = cache:GetGoldAndChanged(player_guid)
	-- 游戏服的话需要清空记录
	if isPk == 0 then
		cache:ClearGoldAndState(player_guid)
	end
	return {ret = 0, msg = 'sync money', gold = gold, changed = changed}
end

-- 跨服回来钱的改变量
function _M:kuafu_restore(player_guid, changed)
	cache:ChangedGold(player_guid, changed)
end
