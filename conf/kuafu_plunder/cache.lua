...
local cache = ngx.shared.kuafu_plunder_cache
...
function _M:SetMatchResult(player_guid, match_result, saveDB)
	...
	if saveDB then
		...
		-- 先判断有没有记录
		local cursor = db.xianfu_match_result_list:find({player_guid=player_guid})
		if cursor:hasNext() then
			if match_result then
				--更新排名信息
				db.xianfu_match_result_list:update({player_guid = player_guid}, {['$set']={result_info=match_result,update_time = os.time()}},true)
			else
				-- 如果是置空的就清掉好了
				db.xianfu_match_result_list:delete({player_guid = player_guid})
			end
		else
			-- 插入信息
			db.xianfu_match_result_list:insert({player_guid=player_guid, result_info=match_result, update_time = os.time()})
		end
	end
end
